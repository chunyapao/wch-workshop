...
📌 ค่า config ที่สำคัญ:
  • adapter_path = "../01_pretraining/adapters" → ใช้ Adapter จาก Pretraining
    ⚠️ ควรเปลี่ยนเป็น "./adapters" เพื่อใช้ Adapter ที่เทรนอีสานแล้ว
  • max_tokens=130 → จำกัดคำตอบแต่ละเส้นทาง 130 Token
    - ถ้าค่าน้อย: คำตอบสั้น อาจไม่ครบถ้วน
    - ถ้าค่ามาก: คำตอบยาว แต่ใช้เวลานาน
    - 130 เหมาะกับการวิเคราะห์ที่ต้องให้เหตุผล

💡 ผลต่อ Inference:
  - ToT = สร้างหลายเส้นทางความคิด แล้วเลือกเส้นทางที่ดีที่สุด
  - max_tokens ที่มากพอ → คำตอบมีเหตุผลครบถ้วน
  - max_tokens ที่น้อยไป → คำตอบถูกตัดกลางคัน เหตุผลไม่ครบ
  - การมีหลายเส้นทางช่วยให้ได้คำตอบที่ดีกว่าการถามครั้งเดียว

⚠️ ข้อจำกัด:
  - ใช้ adapter จาก pretraining → คำตอบอาจไม่อีสาน
  - ควรใช้ "./adapters" ที่เทรนจาก 01_training-run-isan.py
"""
...
branch_1_prompt = """
เจ้าต้องตอบเป็นภาษาอีสานเท่านั้น
...
ข้อยมี 3 เส้นทางความคิดเกี่ยวกับคำถาม: "บุญส่ง ศรีทอง เฮ็ดงานแรกปีได?"
...
2. เพราะอี่หยังถึงเลือกเส้นทางนั้น?
...
จงสรุปคำตอบสุดท้ายเป็นภาษาอีสาน:
- ระบุปีที่เริ่มเฮ็ดงานครั้งแรก
- ให้เหตุผลสั้นๆ
- ตอบให้ชัดเจน

คำตอบสุดท้ายเป็นภาษาอีสาน:
"""
