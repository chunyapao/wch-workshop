"""
สคริปต์นี้แสดงการใช้งาน Majority Voting over Chain-of-Thought

📌 ค่า config ที่สำคัญ:
  • num_votes = 5 → จำนวนรอบที่จะโหวต
    - ถ้าค่าน้อย: ผลลัพธ์ไม่แน่นอน (อาจได้คำตอบผิด)
    - ถ้าค่ามาก: ผลลัพธ์แน่นอนขึ้น แต่ใช้เวลานานและ Token มากขึ้น
    - 5 รอบเป็นค่ามาตรฐานที่สมดุล

💡 ผลต่อ Inference:
  - Majority Voting = รัน CoT หลายๆ ครั้ง แล้วเลือกคำตอบที่ได้บ่อยที่สุด
  - ช่วยลดโอกาสได้คำตอบผิด (เพราะคำตอบผิดมักได้คะแนนน้อย)
  - num_votes ที่มาก → คำตอบแน่นอนขึ้น แต่ใช้ Token มากขึ้น
  - เหมาะกับคำถามที่ต้องการความแม่นยำสูง
"""
